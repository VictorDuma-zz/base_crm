...
"""
It is an educational project but in spite of that it is has deployed and is working now.
"""
...
#List of engineers works that have been paid
...
#List of engineers works that requiring for pay
class PaidEngineer(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['balance'] = Balance.objects.get(name='Main')
    ...
    # via AJAX function PaidEngineer need checkbox to change status for TRUE
...
#List of orders from database with sorting possibility
@login_required
def kvant_base(request):
    engineers = Engineer.objects.all()
    balance = Balance.objects.get(name='Main')
...
#List of transactions from database
@login_required
def finance(request):
...
# function for customer search field in AddOrder class via AJAX. TO DO
@login_required
def search_customer(request):
    if request.method == "POST":
        ...
        search_text = ''
    customers = Customer.objects.filter(contact_name__istartswith=search_text)

    return render_to_response('search_customer.html', {'customers': customers})




#function for find customers financial history to page or excel file
@login_required
def history(request):
...
"""Below functions for creating reports jobs for partners companies"""
@login_required
...
#function for create excel report of engeeners works
@login_required
...
#class for decorating other classes required authorization
...
#classview for form add new order
class AddOrder(LoginRequiredMixin, CreateView):
    ...
    #
    #     if form.is_valid():
    #         customer = Customer.objects.get(contact_name=contact_name)
    #         print contact_name
...
#classview for form add new customer
...
#classview for form edit customer
...
#function for add necessary works for repairing orders device
    def get_context_data(self, **kwargs):
...
# class for create engeeners works sallary
...
# class for edit engeeners works sallary
...
#function for copy order and create new with the last pk number
@login_required
...
# set status close and data of close. If customer pay for repair - create transaction
@login_required
...
#choose  ballance and create debit transaction
@login_required
...
#edit debit transaction with change current amuont of balance
@login_required
...
#edit credit transaction with change current amuont of balance
@login_required
...
#choose  ballance and create credit transaction
@login_required
